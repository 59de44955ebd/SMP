...
SEPARATOR_BRUSH = gdi32.CreateSolidBrush(0xD7D7D7)
...
#    return LPCWSTR(x)
    return cast(c_void_p(x & 0xFFFF), LPCWSTR)
...
def center_window(hwnd, hwnd_parent = None):
    ...
    x = max(0, (rc_parent.left + rc_parent.right - width) // 2)
    y = max(0, (rc_parent.top + rc_parent.bottom - height) // 2)
...
#        self.is_dark = False
        ...
        self.h_font = H_FONT_SHELL
        user32.SendMessageW(self.hwnd, WM_SETFONT, H_FONT_SHELL, MAKELPARAM(1, 0))
