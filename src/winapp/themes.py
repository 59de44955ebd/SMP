...
DARKER_BG_BRUSH = gdi32.CreateSolidBrush(DARKER_BG_COLOR)
...
DARK_TOOLBAR_BORDER_BRUSH = DARK_BORDER_BRUSH
...
class FocusHandler():
    ...
    #
    ########################################
    def handle_focus(self, is_dark):

        if is_dark:
            self.register_message_callback(WM_SETFOCUS, self._on_WM_SETFOCUS)
            self.register_message_callback(WM_KILLFOCUS, self._on_WM_KILLFOCUS)
            self.register_message_callback(WM_NCPAINT, self._on_WM_NCPAINT)
        else:
            self.unregister_message_callback(WM_SETFOCUS, self._on_WM_SETFOCUS)
            self.unregister_message_callback(WM_KILLFOCUS, self._on_WM_KILLFOCUS)
    ...
    def _on_WM_NCPAINT(self, hwnd, wparam, lparam):
        ...
        user32.FrameRect(hdc, byref(rc), HIGHLIGHT_BRUSH if user32.GetFocus() == hwnd else DARK_BORDER_BRUSH)
...
#        print('_on_WM_SETFOCUS')
        if self.old_proc:
            self.old_proc(hwnd, WM_SETFOCUS, wparam, lparam)
        hdc = user32.GetWindowDC(hwnd)
        rc = self.get_window_rect()
        rc = RECT(0, 0, rc.right - rc.left, rc.bottom - rc.top)
...
#        print('_on_WM_KILLFOCUS')
        if self.old_proc:
            self.old_proc(hwnd, WM_KILLFOCUS, wparam, lparam)
        hdc = user32.GetWindowDC(hwnd)
        rc = self.get_window_rect()
        rc = RECT(0, 0, rc.right - rc.left, rc.bottom - rc.top)
